...
class OpenDevAgent:
    ...
    def fork_and_fix_issue(self, session_id: str, issue_number: int) -> dict[str, Any]:
        ...
        try:
            ...
            repo_slug = self.github._extract_repo_slug(state.repo_url)
            is_same_repo = fork_info["full_name"].lower() == repo_slug.lower()

            if is_same_repo:
                fork_full_name = None
                fork_owner = None
                fork_url = self.github._build_clone_url(state.repo_url)
                self._log(state, "info", "fork", f"User owns {repo_slug}, cloning directly.")
            else:
                fork_full_name = fork_info["full_name"]
            ...
            self._log(state, "info", "llm", f"Generating fix via {self.settings.active_llm_provider}...")
            llm_result = self.llm.generate_code(prompt)
            self._log(state, "info", "llm", f"Provider: {llm_result.provider} | {int(llm_result.latency_ms)}ms")
            payload = llm_result.parsed
            changes = payload.get("changes") or []
            if not changes:
                raise AgentError(payload.get("summary", "LLM returned no changes."))
            self._verify_patch_safety(state, repo_path, changes)
            ...
            # Build richer PR body with impact analysis
            impact = payload.get("impact_analysis", "")
            base_body = payload.get("pr_body") or f"## Summary\n{payload.get('summary','')}\n\nCloses #{issue_number}."
            pr_body = base_body
            if impact:
                pr_body += f"\n\n## Impact Analysis\n{impact}"
            pr_body += f"\n\n---\n_Generated by OpenDev AI using {llm_result.provider}_"
            ...
            result = {"action": "fork_fix", "status": "awaiting_approval", "issue_number": issue_number,
                      ...
                      "llm_provider": llm_result.provider, "llm_latency_ms": int(llm_result.latency_ms)}
    ...
    def create_issues_from_findings(self, session_id: str, finding_ids: list[int] | None = None) -> dict[str, Any]:
        ...
        # Fetch existing issue titles to prevent duplicates
        self._log(state, "info", "create_issues", "Checking for existing issues to avoid duplicates…")
        existing_titles = self.github.get_existing_issue_titles(state.repo_url)

        self._log(state, "info", "create_issues", f"Found {len(existing_titles)} existing opendev-ai issues. Processing {len(findings)} findings…")
        created: list[dict[str,Any]] = []; errors: list[str] = []; skipped = 0
        for finding in findings:
            # Build the expected title to check for duplicates
            severity = finding.get("severity", "medium")
            description = finding.get("description", finding.get("type", "unknown").replace("_", " ").title())
            file_path = finding.get("file")
            line = finding.get("line")
            location = f"`{file_path}`" if file_path else "repository"
            if line:
                location += f" line {line}"
            expected_title = f"[{severity.upper()}] {description} in {location}"[:200]

            if expected_title in existing_titles:
                skipped += 1
                self._log(state, "info", "create_issues", f"Skipped (duplicate): {expected_title[:60]}")
                continue
        ...
        if skipped > 0:
            self._log(state, "info", "create_issues", f"Skipped {skipped} duplicate issue(s) already in the repo.")

        return {"status": "ok", "created": created, "errors": errors,
                "issues_created": len(created), "skipped_duplicates": skipped,
                "total_findings": len(state.last_scan_findings)}
    ...
    def _verify_patch_safety(self, state: SessionState, repo_path: Path, changes: list[dict[str, Any]]) -> None:
        """
        Pre-flight safety checks on LLM-generated patches.
        Prevents accidental deletion of critical files and validates
        that changes won't break the codebase.
        """
        CRITICAL_FILES = {"package.json", "requirements.txt", "setup.py", "pyproject.toml",
        ...
        for change in changes:
            ...
            if action == "delete" and Path(rel).name in CRITICAL_FILES:
                raise AgentError(f"Patch safety: refusing to delete critical file '{rel}'")

            # For updates, compare content size to detect accidental truncation
            if action == "update":
                ...
                if target.exists():
                    ...
                    # Warn if patch removes more than 60% of lines
                    if orig_lines > 10 and new_lines < orig_lines * 0.4:
                        warnings.append(
                            f"'{rel}': patch removes {orig_lines - new_lines}/{orig_lines} lines "
                            f"({int((1 - new_lines/orig_lines) * 100)}% reduction)"
                        )

                    # Check if patch removes import statements
                    orig_imports = {l.strip() for l in original.splitlines() if l.strip().startswith(("import ", "from "))}
                    new_imports = {l.strip() for l in (new_content or "").splitlines() if l.strip().startswith(("import ", "from "))}
                    removed_imports = orig_imports - new_imports
                    if len(removed_imports) > 3:
                        warnings.append(f"'{rel}': patch removes {len(removed_imports)} import statements")

        if warnings:
            for w in warnings:
                self._log(state, "warn", "patch_safety", f"WARNING: {w}")
            self._log(state, "warn", "patch_safety",
                       "Patch has safety warnings but proceeding. Review the diff carefully before approving.")
