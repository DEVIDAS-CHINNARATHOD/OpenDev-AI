"""
main.py — OpenDev AI v5 FastAPI application
...
# ── CORS ────────────────────────────────────────────────────────────────────────
# All origins come entirely from environment variables (FRONTEND_ORIGIN /
# FRONTEND_ORIGIN_REGEX).  No localhost defaults — configure .env explicitly.
_origins = settings.frontend_origins
_regex = settings.frontend_origin_regex or None
...
def root() -> dict:
    return {
        ...
        "endpoints": [
            "POST /repo", "GET /issues", "POST /fork-fix",
            "POST /scan", "POST /create-issues", "POST /pr-review",
            "POST /approve", "GET /logs", "GET /security-score",
            "GET /rl-stats", "POST /pr-feedback",
        ],
    }
...
def load_repo(payload: RepoRequest) -> dict:
    try:
        return agent.create_repo_session(payload.repo_url)
...
def get_issues(session_id: str = Query(..., min_length=1)) -> dict:
    try:
        return agent.get_issues(session_id)
    except AgentError as exc:
...
def fork_fix(payload: FixRequest) -> dict:
    try:
        return agent.fork_and_fix_issue(payload.session_id, payload.issue_number)
    except AgentError as exc:
        raise HTTPException(400, detail=str(exc)) from exc
...
def fix_issue(payload: FixRequest) -> dict:
    try:
        return agent.fix_issue(payload.session_id, payload.issue_number)
    except AgentError as exc:
        raise HTTPException(400, detail=str(exc)) from exc
...
def run_scan(payload: ScanRequest) -> dict:
    try:
        return agent.deep_security_scan(payload.session_id)
    except AgentError as exc:
        raise HTTPException(400, detail=str(exc)) from exc
...
def create_issues(payload: CreateIssuesRequest) -> dict:
    try:
        return agent.create_issues_from_findings(payload.session_id, payload.finding_ids)
    except AgentError as exc:
        raise HTTPException(400, detail=str(exc)) from exc
...
def pr_review(payload: PRReviewRequest) -> dict:
    try:
        return agent.review_pull_request(
            payload.session_id, payload.repo_url, payload.pr_number, payload.post_comment
        )
    except AgentError as exc:
        raise HTTPException(400, detail=str(exc)) from exc
...
def approve(payload: ApprovalRequest) -> dict:
    try:
        return agent.approve(payload.session_id, payload.approved)
    except AgentError as exc:
        raise HTTPException(400, detail=str(exc)) from exc
...
def get_logs(session_id: str = Query(..., min_length=1)) -> dict:
    try:
        return agent.get_logs(session_id)
    except AgentError as exc:
        raise HTTPException(404, detail=str(exc)) from exc
...
def security_score(session_id: str = Query(..., min_length=1)) -> dict:
    try:
        return agent.get_security_score(session_id)
    except AgentError as exc:
        raise HTTPException(404, detail=str(exc)) from exc
...
def pr_feedback(payload: PRFeedbackRequest) -> dict:
    try:
        exec_result = {
            ...
            "tests_failed": not payload.tests_passed and not payload.build_failed,
            "build_failed": payload.build_failed,
            "issue_fixed": payload.issue_closed,
            "secret_removed": False,
            "no_change": False,
        }
        pr_status = {
            "merged": payload.pr_merged,
            "state": "closed" if payload.issue_closed else "open",
            "issue_closed": payload.issue_closed,
        }
...
def terminate(payload: SessionRequest) -> dict:
    try:
        return agent.terminate_session(payload.session_id)
    except AgentError as exc:
        raise HTTPException(404, detail=str(exc)) from exc
